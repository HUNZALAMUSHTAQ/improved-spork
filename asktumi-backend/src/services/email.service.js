...
const sendSetPasswordEmail = async (to, token) => {
  ...
  const __dirname = path.resolve();
  const filePath = path.join(`${__dirname}/src/email-templates/set-password.hbs`);
  const source = fs.readFileSync(filePath, 'utf-8').toString();
  const template = handlebars.compile(source);

  const resetPasswordUrl = `${process.env.FRONTEND_BASE_URL}/pages/authentication/set-password?token=${token}`;
  const subject = 'Reset password';
